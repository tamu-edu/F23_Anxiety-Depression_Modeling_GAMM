...
Anxiety_Depres <- Anxiety_Depres %>%
  select(`Participant ID`,`T-score`,`Standard Error`, `Complete?`, MeanTrait_STAI, AnxietyGrp, DepresGroup )
...
M1 <- glmer(altBinomial ~ blocks.thisN + AnxietyGrp + DepressGrp + prevOutcome + prevCondition + rewCond + (1|participant), data = conjoinedBehav_AnxDepres, family = binomial(link = "logit"), na.action = na.omit)
summary(M1)
...
library(glmmTMB)
library(sjPlot)
plot_model(M1, type = "re", show.values = FALSE, value.offset = 2, vline.color = "red") #plots the exponnentiated regression estimates (odds ratio)

#probability transform
plot_model(M1,type ="re", transform = "plogis", show.values = FALSE) #probaility transform for re
```

#plot fixed effects
```{r fixed effects, echo=FALSE}
plot_model(M1, type = "est", transform = "plogis", vline.color = "red", show.values = TRUE, value.offset = .3)
```
